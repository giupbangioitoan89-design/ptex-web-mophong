...
## 4. Cập Nhật Mới Nhất & Hoàn Thiện Bài 1 (Góc Lượng Giác - Demo 2.1 đến 2.5)

Chúng tôi đã hoàn thiện triệt để bài học 1 này bằng các cập nhật quan trọng:

### A. Khắc phục lỗi hiển thị công thức KaTeX bị sọc dọc (Vertical Text)
* **Triệu chứng**: Công thức toán ở chân trang (ví dụ $\alpha = \theta + k \cdot 2\pi ...$) bị ngắt dòng liên tiếp thành từng từ xếp chồng dọc như `alpha =`, `theta + k`, `cdot2`, `pi`...
* **Nguyên nhân**: 
  - Trong tệp seed dữ liệu [route.ts](file:///d:/PTex2026/ptex-web-mophong/src/app/api/seed/route.ts), chuỗi `mathContent` bị over-escaped bằng 4 dấu gạch chéo ngược (`\\\\`) thay vì 2 (`\\`).
  - Khi truyền qua API và parse ở client, nó trở thành hai dấu gạch chéo ngược literal (`\\`). Trong cú pháp LaTeX/KaTeX, `\\` là lệnh ngắt dòng (newline), khiến KaTeX tự động ngắt dòng và render phần còn lại dưới dạng chữ thường không định dạng.
* **Cách khắc phục**:
  1. Đưa các định nghĩa trong [route.ts](file:///d:/PTex2026/ptex-web-mophong/src/app/api/seed/route.ts) về đúng 2 dấu gạch chéo ngược (`\\alpha`, `\\theta`, `\\equiv`, `\\pmod`).
  2. Bổ sung bộ lọc chuẩn hóa ở client tại [MathRenderer.tsx](file:///d:/PTex2026/ptex-web-mophong/src/components/MathRenderer.tsx) và tại hàm `math` trong [SimulationBoard.tsx](file:///d:/PTex2026/ptex-web-mophong/src/components/SimulationBoard.tsx) để tự động chuyển `\\\\` (double backslashes trong chuỗi literal) về `\\` (single backslash) trước khi truyền cho KaTeX. Điều này giúp hệ thống chống lỗi và hiển thị công thức đúng hàng ngang thẳng tắp.

### B. Đồng bộ mô hình Xe đạp & Bánh xe lăn trên đường (Demo 2.3)
* **Khôi phục hoàn hảo**: Đảm bảo chế độ bánh xe lăn tịnh tiến vẽ đường cong Cycloid đỏ, cùngnan hoa và nắp trục tự quay tròn, kết nối gióng khung xe đạp thể thao Indigo và bánh trước chuyển động nhịp nhàng.
* **Chặn tương tác trực tiếp điểm M**: Fixed vị trí điểm M ở hai chế độ chuyển động thực tế này để học sinh chỉ thao tác qua thanh trượt hoặc Autoplay, ngăn chặn việc tính toán sai lệch góc quay do tâm bánh xe di động.
* **Aspect-Ratio & Flickering**: Đảm bảo bánh xe luôn tròn xoe trên PC/Mobile (sử dụng tính toán tỷ lệ canvas động của board) và loại bỏ hoàn toàn việc nháy nhãn (Flicker) khi chạy autoplay bằng cách gom điều kiện chỉ chạy `setBoundingBox` khi kích thước khung vẽ hoặc chế độ thực sự thay đổi.

### C. Chuẩn hóa hiển thị 2 chữ số thập phân (Decimal Precision)
* **Vấn đề**: Các thông số hiển thị lúc có 3 chữ số thập phân, lúc có nhiều chữ số lẻ thập phân do sai số số thực JS gây rối mắt.
* **Giải pháp**: 
  - Cập nhật toàn bộ các readouts còn lại từ `.toFixed(3)` thành `.toFixed(2)` ở các Demos trong `route.ts`.
  - Áp dụng bộ lọc `parseFloat(deg.toFixed(2))` cho các góc lượng giác.
  - Các chỉ số sin, cos, tan, cot, góc độ, bán kính R, quãng đường lăn, biểu thức tính toán hiện nay hiển thị rất đồng bộ và sạch sẽ với tối đa 2 chữ số thập phân.
